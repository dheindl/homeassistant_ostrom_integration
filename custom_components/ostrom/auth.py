...
async def get_access_token(client_id: str, client_secret: str, environment: str) -> Optional[Dict[str, Any]]:
    """Get access token from Ostrom API. Retries once on HTTP 429."""
            ...
            try:
                async with session.post(auth_url, auth=auth, data=payload) as response:
                    if response.status == 429:
                        ...
                        if attempt == 0:
                            _LOGGER.warning(
                                "Ostrom auth API rate-limited (429), retrying in %s s", retry_after
                            )
                            await asyncio.sleep(retry_after)
                            continue
                        _LOGGER.error("Ostrom auth API still rate-limited after retry, giving up")
                        return None
